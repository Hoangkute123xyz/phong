...
class PostDetailFragment : MVVMBaseFragment<FragmentPostDetailBinding, PostDetailViewModel>(),
        ...
        postDetailViewModel.productListResponse.observe(this, Observer { response ->
            if (viewLifecycleOwner.lifecycle.currentState == Lifecycle.State.RESUMED) {
                if (response.data.isNotEmpty()) {
                    ...
                    cvItemList.visibility = View.GONE
                }
            }
        })
    }
    ...
    private fun getResponse(post: Post) {
        ...
        postListDetailDate.text =
            getString(R.string.post_detail_author_template, post.userName, date)
        // Address
        ...
        if (post.serviceId == Constant.JOB_SERVICE_ID) {
            // Get job type
            titlePostDetailJobType.apply {
                visibility = View.VISIBLE
                text = getString(R.string.title_job_type_template)
            }
            postDetailViewModel.requestJobType()

            // Get job category
            titlePostDetailJobCategory.apply {
                visibility = View.VISIBLE
                text = getString(R.string.title_job_category_template)
            }
            ...
            titlePostDetailJobType.visibility = View.GONE
            postDetailJobType.visibility = View.GONE
            titlePostDetailJobCategory.visibility = View.GONE
        ...
        if (isToken()) {
            when {
                post.price != "0" || post.salary != "0" -> {
                    ...
                    // detailPostPrice.setTextColor(getColor(R.color.color_indigo_de608c))
                }
            }
        } else {
        ...
        if (post.phone.isNotEmpty()) {
            //title
            titlePostDetailPhone.visibility = View.VISIBLE
            titlePostDetailPhone.text = getString(R.string.title_phone)
            //detail
            ...
            titlePostDetailPhone.visibility = View.GONE
        ...
        postDetailCmtLink.text =
            getString(R.string.comment_title_count, post.comments.size.toString())
    ...
    private fun processPriceUnit(post: Post): CharSequence? {
        return when (post.serviceId) {
            ...
            Constant.ADS_SERVICE_ID -> resources.getString(
                R.string.dynamic_money_unit,
                String.format("%,d", WindyConvertUtil.filterNumeric(post.price).toInt())
            )
            Constant.TRAVEL_SERVICE_ID -> resources.getString(
                R.string.dynamic_price_per_people,
                String.format("%,d", WindyConvertUtil.filterNumeric(post.price).toInt())
            )
            Constant.TRANSLATOR_SERVICE_ID -> getPriceUnitByType(post)
            Constant.CAR_SERVICE_ID -> resources.getString(
                R.string.dynamic_money_per_month,
                String.format("%,d", WindyConvertUtil.filterNumeric(post.price).toInt())
            ) + if (!TextUtils.isEmpty(post.area)) {
                String.format("\n%s m²", post.area)
            } else {
                ""
            }
    ...
    private fun getPriceUnitByJobType(post: Post): CharSequence? {
        ...
        return resources.getString(
            a,
            String.format("%,d", WindyConvertUtil.filterNumeric(post.salary).toInt())
        )
    }
    ...
    override fun onClick(v: View?) {
        when (v?.id) {
            R.id.postDetailCmtLink -> {
                val action =
                    PostDetailFragmentDirections.actionPostDetailFragmentToCommentFragment(postId)
                findNavController().navigate(action)
            }
            R.id.postDetailFavLink -> {
                val action =
                    PostDetailFragmentDirections.actionPostDetailFragmentToLikedListFragment(postId)
                findNavController().navigate(action)
            }
            ...
            R.id.detailMsgBtn -> {
                if (isToken()) {
                    val action = PostDetailFragmentDirections
                        .actionPostDetailFragmentToChatDetailFragment(
                            postData.userId,
                            postData.userName,
                            postData.userAvatar,
                            postData.phone
    ...
    private fun specificEditDestination(serviceId: String) {
        when (serviceId) {
            "13" -> findNavController().navigate(
                PostDetailFragmentDirections.actionPostDetailFragmentToMartPostFragment(
                    postId,
                    Constant.MODE_EDIT
                )
            )
            "12" -> findNavController().navigate(
                PostDetailFragmentDirections.actionPostDetailFragmentToJobPostFragment(
                    postId,
                    Constant.MODE_EDIT
                )
            )
            "11" -> findNavController().navigate(
                PostDetailFragmentDirections.actionPostDetailFragmentToTransPostFragment(
                    postId,
                    Constant.MODE_EDIT
                )
            )
            "9" -> findNavController().navigate(
                PostDetailFragmentDirections.actionPostDetailFragmentToTravelPostFragment(
                    postId,
                    Constant.MODE_EDIT
                )
            )
            "8" -> findNavController().navigate(
                PostDetailFragmentDirections.actionPostDetailFragmentToCarPostFragment(
                    postId,
                    Constant.MODE_EDIT
                )
            )
            "7" -> findNavController().navigate(
                PostDetailFragmentDirections.actionPostDetailFragmentToRestaurantPostFragment(
                    postId,
                    Constant.MODE_EDIT
                )
            )
            "6" -> findNavController().navigate(
                PostDetailFragmentDirections.actionPostDetailFragmentToSpaPostFragment(
                    postId,
                    Constant.MODE_EDIT
                )
            )
            "5" -> findNavController().navigate(
                PostDetailFragmentDirections.actionPostDetailFragmentToPhonePostFragment(
                    postId,
                    Constant.MODE_EDIT
                )
            )
            "4" -> findNavController().navigate(
                PostDetailFragmentDirections.actionPostDetailFragmentToVisaPostFragment(
                    postId,
                    Constant.MODE_EDIT
                )
            )
            "1" -> findNavController().navigate(
                PostDetailFragmentDirections.actionPostDetailFragmentToAdsPostFragment(
                    postId,
                    Constant.MODE_EDIT
                )
            )
    ...
    private fun onClickImg(list: ArrayList<String>, position: Int) {
        ...
        val action =
            PostDetailFragmentDirections.actionPostDetailFragmentToZoomFragment(listImg, position)
        findNavController().navigate(action)
    }
    ...
    override fun onItemClick(position: Int) {
        val images  = Array(1){productAdapter.list[position].img}
        val action = PostDetailFragmentDirections.actionPostDetailFragmentToZoomFragment(images, 0)
        findNavController().navigate(action)
    }
}