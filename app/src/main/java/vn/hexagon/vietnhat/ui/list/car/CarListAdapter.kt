...
class CarListAdapter(
    ...
    override fun bind(binding: ViewDataBinding, position: Int) {
        when (binding) {
            is LayoutItemListServiceCarBinding -> {
                ...
                binding.itemLastText.getPhoneNumber(position)
                ...
                if(!TextUtils.isEmpty(currentList[position].area)) {
                    binding.tvArea.text = String.format("%s m²", currentList[position].area)
                }else{
                    binding.tvArea.text = "Không xác định"
                }
            }
    ...
    private fun TextView.getPhoneNumber(position: Int) {
        this.text = currentList[position].phone
    }
