package vn.hexagon.vietnhat.ui.list.fonegift
...
import vn.hexagon.vietnhat.databinding.FragmentFoneGiftListBinding
 ...
 * =====================================================
 */
class FoneGiftListFragment : MVVMBaseFragment<FragmentFoneGiftListBinding, PostListViewModel>(),
    ...
    override fun isShowActionBar(): View? = SimpleActionBar(activity).apply {
        simpleTitleText = getString(R.string.create_post_phone_gift)
        ...
        rightActionBarButton.setOnClickListener {
            fragmentManager?.let {
                ...
                dialog.setTargetFragment(this@FoneGiftListFragment, 0)
    ...
    override fun getLayoutId(): Int = R.layout.fragment_fone_gift_list

    override fun initView() {
        getBaseViewDataBinding().mainviewmodel = (activity as MainActivity).mainViewModel
        // init adapter
        val phoneAdapter = FoneGiftListAdapter(postListViewModel, ::onItemClick, ::onClickFav)
        // Refresh layout
        ...
        // Response data
        postListViewModel.foneGiftResponse.observe(this, Observer { response ->
...
//        })
    ...
    private fun loadData(numberPost: Int) {
        postListViewModel.getPhoneGift(
            Constant.INDEX,
            numberPost
        )
    }
     ...
     * @param response
     */
    private fun getResponse(response: PhoneGiftResponse) {
     ...
     * @param postId
     */
    private fun onItemClick(postId: String) {
        val action =
            FoneGiftListFragmentDirections.actionFoneGiftListFragmentToPostFoneGiftDetailFragment(
                postListViewModel.userId,
                postId
            )
