 ...
 */
class LoginFragment : MVVMBaseFragment<FragmentLoginBinding, LoginFragmentViewModel>(),
    ...
    override fun initData(argument: Bundle?) {
        if (argument != null) {
            var isLogout = false
            try {
                isLogout = LoginFragmentArgs.fromBundle(argument).isLogout
    ...
    override fun initView() {
        activity?.let { context ->
            ...
            val phone =
                sharedPref.getString(getString(R.string.variable_local_phone), Constant.BLANK)
            val password =
                sharedPref.getString(getString(R.string.variable_local_password), Constant.BLANK)
            val countryCd =
                sharedPref.getString(getString(R.string.variable_local_country_cd), Constant.BLANK)
            ...
            loginForgotLink.setOnClickListener {
                if (edPhone.text.toString().isEmpty()) {
                    ...
                    val action =
                        LoginFragmentDirections.actionLoginFragmentToRequestSmsCodeFragment(
                            WindyConvertUtil.convertPhoneNumber(countryCodePicker, edPhone)
                        )
        ...
        when {
            ...
            !Utils.isValidPhoneNumber(countryCodePicker.selectedCountryCodeWithPlus + edPhone.text.toString()) -> edPhone.apply {
                ...
                loginViewModel.doLogin(
                    WindyConvertUtil.convertPhoneNumberTrim(countryCodePicker, edPhone),
    ...
    private fun getResponse(response: LoginResponse) {
        ...
        if (response.errorId == RESPOND_CD) {
            ...
            //Save user avatar_url
            saveToLocal(R.string.variable_local_avt, response.user.avatar)
            //Save user name
            saveToLocal(R.string.variable_local_name, response.user.name)
