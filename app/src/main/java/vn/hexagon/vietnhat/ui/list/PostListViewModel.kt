 ...
 */
class PostListViewModel @Inject constructor(val repository: PostListRepository) :
    MVVMBaseViewModel() {
    /** Post list response */
    ...
    /*Fone response*/
    val foneListResponse = MutableLiveData<ListFoneHouseResponse>()

    /*Fone search response*/
    val foneSearchResponse = MutableLiveData<ListFoneHouseResponse>()

    /*Fone Gift response*/
    val foneGiftResponse = MutableLiveData<PhoneGiftResponse>()
    ...
    fun getDataListPost(
        ...
        val fus = LocationServices.getFusedLocationProviderClient(context)
        fus.lastLocation.addOnCompleteListener {
            if (it.isSuccessful && it.result != null) {
                ...
                getListPost(userId, serviceId, index, number, null, null)
            }
        }
    ...
    fun getListPost(
        ...
        repository.getPostList(userId, serviceId, index, number, lat, lng)
            .applyScheduler()
            .subscribe(
                { result ->
    ...
    fun addFavouriteGiftRequest(userId: String, id: String) {
        // Start loading
        networkState.postValue(NetworkState.LOADING)
        // Send request
        repository.addFavouriteGift(userId, id)
            .applyScheduler()
            .subscribe(
                { result ->
                    networkState.postValue(LOADED)
    ...
    /**
     * Get Phone House List
     *
     * @param userId
     * @param id
     */
    fun getFoneHouse(index: Int, number: Int) {
        // Start loading
        networkState.postValue(NetworkState.LOADING)
        // Send request
        repository.getFoneHouse(index, number)
            .applyScheduler()
            .subscribe(
                { result ->
                    networkState.postValue(LOADED)
                    foneListResponse.postValue(result)
                },
                { throwable ->
                    networkState.postValue(NetworkState.ERROR)
                    DebugLog.e(throwable.message.toString())
    ...
    /**
     * Search Phone House List
     *
     * @param brandID
     * @param modelID
     */
    fun searchPhoneHouse(brandID: String, modelID: String, numberPost: Int) {




//        var response = foneListResponse.value
//        var wrapperListPhoneHouse = arrayListOf<Fone>()
//
//        response?.data?.let {
//            for (item in it) {
//                if (item.brandId == brandID && item.typeId == modelID) {
//                    wrapperListPhoneHouse.add(item)
//                }
//            }
//
//            response.data = wrapperListPhoneHouse
//            foneListResponse.postValue(response)
//        }

        // Start loading
        networkState.postValue(NetworkState.LOADING)
        // Send request
        repository.searchPhoneHouse(brandID, modelID, numberPost)
            .applyScheduler()
            .subscribe(
                { result ->
                    networkState.postValue(LOADED)
                    foneSearchResponse.postValue(result)
                },
                { throwable ->
                    networkState.postValue(NetworkState.ERROR)
                    DebugLog.e(throwable.message.toString())
    ...
    fun removeFavouriteGiftRequest(userId: String, id: String) {
        // Start loading
        networkState.postValue(NetworkState.LOADING)
        // Send request
        repository.removeFavouriteGift(userId, id)
            .applyScheduler()
            .subscribe(
                { result ->
                    networkState.postValue(LOADED)
     ...
     * @param numberPost
     */
    fun getPhoneGift(index: Int, numberPost: Int) {
        // Start loading
        networkState.postValue(NetworkState.LOADING)
        // Send request
        repository.getPhoneGift(index, numberPost)
            .applyScheduler()
            .subscribe({ result ->
                networkState.postValue(LOADED)
                foneGiftResponse.postValue(result)
            },
                { throwable ->
                    networkState.postValue(NetworkState.ERROR)
                    DebugLog.e(throwable.message.toString())
    ...
    fun getFilterPhone(brandID: String, typeID: String,index: Int,numberPost: Int) {
        networkState.postValue(NetworkState.LOADING)
        repository.getFilterPhone(brandID, typeID,index,numberPost)
            .applyScheduler()
            .subscribe({ result ->
                networkState.postValue(LOADED)
                foneSearchResponse.postValue(result)
