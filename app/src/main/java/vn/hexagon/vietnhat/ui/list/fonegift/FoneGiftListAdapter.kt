...
import vn.hexagon.vietnhat.data.model.fone.PhoneGift
import vn.hexagon.vietnhat.data.remote.NetworkState
import vn.hexagon.vietnhat.databinding.LayoutItemListServiceFoneBinding
...
class FoneGiftListAdapter(
    ...
    override fun createBinding(parent: ViewGroup, viewType: Int): ViewDataBinding {
        return if (viewType == Constant.LIST_ITEM_TYPE) {
            DataBindingUtil.inflate(
                ...
                R.layout.layout_item_list_service_fone,
                parent,
                false
            )
        } else {
    ...
    override fun bind(binding: ViewDataBinding, position: Int) {
        when (binding) {
            is LayoutItemListServiceFoneBinding -> {
                ...
                binding.iconFavourite.setOnClickListener {
                    if (currentList[position].isFavourite == 0 && !isAdded) {
                        ...
                        viewModel.addFavouriteGiftRequest(viewModel.userId, currentList[position].id!!)
                        currentList[position].isFavourite = 1
                    } else {
                        isAdded = false
                        binding.iconFavourite.setImageResource(R.drawable.ic_save)
                        viewModel.removeFavouriteGiftRequest(viewModel.userId, currentList[position].id!!)
                ...
                binding.phoneListItemArea.setOnClickListener {
                    getItem(position).id?.let { it1 -> onClick(it1) }
                }
            }
    ...
    companion object {
        class ListDiffCallback : DiffUtil.ItemCallback<PhoneGift>() {
            override fun areItemsTheSame(oldItem: PhoneGift, newItem: PhoneGift): Boolean {
                return oldItem == newItem
            }

            override fun areContentsTheSame(oldItem: PhoneGift, newItem: PhoneGift): Boolean {
                return oldItem == newItem
            }
        }
    }
