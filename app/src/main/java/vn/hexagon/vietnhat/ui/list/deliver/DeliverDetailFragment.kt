...
class DeliverDetailFragment : MVVMBaseFragment<FragmentDetailDeliverBinding, PostDetailViewModel>(),
        ...
        actionBar?.apply {
            ...
            rightActionBarButton.setOnClickListener {
                findNavController().navigate(
                    DeliverDetailFragmentDirections.actionDeliverDetailFragmentToDeliverPostFragment(
                        postId,
                        Constant.MODE_EDIT
                    )
                )
    ...
    private fun getResponse(post: Post) {
        ...
        // Product type
//        detailProductType.text = getString(R.string.detail_product_type, post.productType) //đã thêm TextView + Xóa source String
        detailProductType.text = post.productType
        // Product weight
//        detailProductWeight.text = getString(R.string.detail_product_weight, post.weight)
        detailProductWeight.text = post.weight
        // Phone
        authorPhone = post.phone
//        detailPhone.text = getString(R.string.title_and_detail_phone, post.phone)
        detailPhone.text = post.phone
        // Company name
//        detailCompName.text = getString(R.string.detail_company_name, post.companyName)
        detailCompName.text = post.companyName
        // note
        ...
        detailTakenAddress.setOnClickListener(this)
        detailReceiveAddress.setOnClickListener(this)
        // OnClick fav button
        var isAdded = false
    ...
    override fun onClick(v: View?) {
        when (v?.id) {
            R.id.deliverDetailCmtLink -> {
                val action =
                    DeliverDetailFragmentDirections.actionDeliverDetailFragmentToCommentFragment(
                        postId
                    )
            ...
            R.id.deliverDetailFavLink -> {
                val action =
                    DeliverDetailFragmentDirections.actionDeliverDetailFragmentToLikedListFragment(
                        postId
                    )
                findNavController().navigate(action)
            }
            ...
            R.id.deliverDetailMsgBtn -> {
                if (isToken()) {
                    val action = DeliverDetailFragmentDirections
                        .actionDeliverDetailFragmentToChatDetailFragment(
                            postData.userId,
                            postData.userName,
                            postData.userAvatar,
                            postData.phone
            ...
            R.id.detailTakenAddress -> detailTakenAddress.openGoogleMap()
            R.id.detailReceiveAddress -> detailReceiveAddress.openGoogleMap()
        }
    }
    ...
    private fun onClickImg(list: ArrayList<String>, position: Int) {
        ...
        val action = DeliverDetailFragmentDirections.actionDeliverDetailFragmentToZoomFragment(
            listImg,
            position
        )
        findNavController().navigate(action)
    ...
    private fun PagerDetailAdapter.initPager(list: List<Images>) {
        ...
        // Display indicator
        displayIndicator(homeInfinityIndicator, deliverDetailPager, this.itemCount)
    }
